...
import { ChevronRight, Save, AlertCircle, Building2, Briefcase, GraduationCap, DollarSign, Calendar, Clock, Star, CheckCircle, TrendingUp, Plus } from 'lucide-react'
...
function ExperienceMetadataForm() {
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        ...
        if (res.data.success) {
          // Merge backend companies with global list and remove duplicates
          const backendCompanies = res.data.companies || []
          const backendRoles = res.data.roles || []

          const uniqueCompanies = [...new Set([...GLOBAL_COMPANIES, ...backendCompanies])].sort()
          const uniqueRoles = [...new Set([...GLOBAL_ROLES, ...backendRoles])].sort()
        ...
        // Fallback to global lists if API fails
        setCompanies(GLOBAL_COMPANIES)
        setRoles(GLOBAL_ROLES)
      }
    }
    fetchOptions()
  }, [])
  ...
  useEffect(() => {
    const loadDraft = async () => {
      try {
        const locationStateId = location.state?.experienceId; // If we navigated from "My Experiences" -> Edit
        // But location is not imported yet, we need useLocation.
        // Actually, let's prioritize checking if backend has a draft if no explicit ID provided.

        // Wait, I need useLocation to support Edit flow properly.
        // But for "resume form application", getDraft() is what we want.
        ...
        if (res.data.success && res.data.draft) {
          ...
          setFormData({
            ...
            roleAppliedFor: draft.roleAppliedFor || '',
            batch: draft.batch || new Date().getFullYear().toString(),
            package: draft.package || '',
            placementSeason: draft.placementSeason || 'on-campus',
            interviewYear: draft.interviewYear || new Date().getFullYear().toString(),
            interviewMonth: draft.interviewMonth || (new Date().getMonth() + 1).toString().padStart(2, '0'),
            difficultyRating: draft.difficultyRating || '3',
            preparationTime: draft.preparationTime || '',
            overallExperienceRating: draft.overallExperienceRating || '3',
            outcome: draft.outcome || 'in-process',
          });
          setExperienceId(draft._id);
          setSuccess(`Resumed draft for ${draft.companyName}`);
    ...
    // Only load draft if we don't effectively have an ID yet? 
    // Or just always check on mount.
    if (!experienceId) {
      loadDraft();
    }
  }, []);
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="bg-gradient-to-br from-background to-white rounded-xl p-5 border border-gray-200 hover:border-accent transition-all">
                      ...
                      <div className="relative">
                        ...
                        />
                        {showCompanyDropdown && (
                          <div className="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-accent rounded-xl shadow-xl z-20 max-h-60 overflow-y-auto animate-in fade-in zoom-in-95 duration-200">
                            {(() => {
                              const filtered = companies.filter((c) =>
                                c.toLowerCase().includes(formData.companyName.toLowerCase())
                              );

                              const exactMatch = companies.some(c => c.toLowerCase() === formData.companyName.toLowerCase());
                              ...
                              return (
                                <>
                                  ...
                                  {/* Show "Add new" option if typed text is not empty and not in list (exact match) */}
                                  {formData.companyName && !exactMatch && (
                                    <button
                                      type="button"
                                      onMouseDown={() => setShowCompanyDropdown(false)}
                                      className="w-full text-left px-5 py-3 bg-secondary/10 hover:bg-secondary/20 transition-all font-bold text-primary flex items-center gap-2"
                                    >
                                      <Plus size={16} />
                                      Add "{formData.companyName}"
                                    </button>
                                  )}

                                  {filtered.length === 0 && !formData.companyName && (
                                    <div className="px-5 py-3 text-gray-500 text-sm italic">Type to search or add...</div>
                                  )}
                                </>
                              );
                            })()}
                        ...
                        {showRoleDropdown && (
                          <div className="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-accent rounded-xl shadow-xl z-20 max-h-60 overflow-y-auto animate-in fade-in zoom-in-95 duration-200">
                            {(() => {
                              ...
                              const exactMatch = roles.some(r => r.toLowerCase() === formData.roleAppliedFor.toLowerCase());
                              ...
                              return (
                                <>
                                  ...
                                  {formData.roleAppliedFor && !exactMatch && (
                                    <button
                                      type="button"
                                      onMouseDown={() => setShowRoleDropdown(false)}
                                      className="w-full text-left px-5 py-3 bg-secondary/10 hover:bg-secondary/20 transition-all font-bold text-primary flex items-center gap-2"
                                    >
                                      <Plus size={16} />
                                      Add "{formData.roleAppliedFor}"
                                    </button>
                                  )}

                                  {filtered.length === 0 && !formData.roleAppliedFor && (
                                    <div className="px-5 py-3 text-gray-500 text-sm italic">Type to search or add...</div>
                                  )}
                                </>
                              );
